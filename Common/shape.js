...
function shape()
	...
	shape.prototype.spherical_texture_coords = function( vert_index, tileNum )
		{
			var texCoord1 = .5 + tileNum*Math.atan2( this.vertices[vert_index][0], this.vertices[vert_index][2] ) / (2*Math.PI);
			var texCoord2 = .5 + tileNum*Math.asin( this.vertices[vert_index][1] ) / Math.PI ;
			...
			this.texture_coords.push(vec2(texCoord1, texCoord2));
		};
	...
	shape.prototype.update_uniforms = function( model_transform, camera_transform, projection_transform )			// Send the current matrices to the shader
		{
				...
				//gl.uniform4fv( g_addrs.lightPosition_loc, 	vec4( -1,1,1,0 ) );
...
function sphere( points_transform, max_subdivisions, tileNum=1 )		// Build a sphere using subdivision, starting with a tetrahedron.  Store each level of detail in separate index lists.
	{	
		shape.call(this);	
		...
		this.populate( this, points_transform, max_subdivisions, tileNum );
...
inherit(sphere, shape);
	...
	sphere.prototype.populate = function ( recipient, points_transform, max_subdivisions, tileNum ) 
		{	
			...
			{
				recipient.spherical_texture_coords( i, tileNum );
...
function half_sphere( points_transform, max_subdivisions, tileNum=1 )		// Build a half_sphere using subdivision, starting with a tetrahedron.  Store each level of detail in separate index lists.
{	
	shape.call(this);	
...
half_sphere.prototype.populate = function ( recipient, points_transform, max_subdivisions, tileNum ) 
	{	
		var offset = recipient.vertices.length;
		recipient.vertices.push(		vec3(0.0, 0.0, -1.0) 				 );
		recipient.vertices.push(		vec3(0.0, 0.942809, 0.333333) 		 );
...
function pyramid( points_transform, tileNum=1 )
{	
    // Pyramid with center of base at the origin
    shape.call(this);
    this.populate( this, mat4(), tileNum);
    this.init_buffers();
}
inherit(pyramid, shape);

pyramid.prototype.populate = function ( recipient, points_transform, tileNum ) 
{	
    var m_strip = new triangle_strip();
    var m_square = new rectangular_strip();
	...
	vertices.push( vec3(0.5, 0.0, 0.5) );
	vertices.push( vec3(0.0, 0.866025, 0.0) );
    // Side 2
	vertices.push( vec3(0.5, 0.0, 0.5) );
	vertices.push( vec3(0.5, 0.0, -0.5) );
	vertices.push( vec3(0.0, 0.866025, 0.0) );
    ...
    for (var i = 0, t = 0; i < 12; i+=3, t++) {     // Make 4 triangles for sides
        m_strip.init_from_strip_lists( recipient, [vertices[i],vertices[i+1],vertices[i+2]], indices );
        recipient.texture_coords[i]   = vec2(t*0.25*tileNum,0);
        recipient.texture_coords[i+1] = vec2((t+1)*.25*tileNum,0);
        recipient.texture_coords[i+2] = vec2((t*.25+.125)*tileNum,1);
    }
    m_square.populate( recipient, 1, mult(mat4(),rotate(90,0,0,1)));	// Make 1 square for bottom
...
function rectangular_strip( numRectangles, points_transform, tileNum )
	{
		triangle_strip.call(this);	
		if( !arguments.length) return;	// Pass no arguments if you just want to statically call functions up the inheritance chain, and / or populate()
		this.populate( this, numRectangles, points_transform, tileNum );
		this.init_buffers();
	}
inherit(rectangular_strip, triangle_strip);

	rectangular_strip.prototype.populate = function( recipient, numRectangles, points_transform, tileNumX, tileNumZ )	
				{	
					//var tileNum = 10;
					...
					{
						vertices[topIdx] 	= vec3( 0,  .5, topIdx - .5 * numRectangles );		recipient.texture_coords[ topIdx + offset ]    = vec2( tileNumZ*topIdx / numRectangles, tileNumX );
						vertices[bottomIdx] = vec3( 0, -.5, topIdx - .5 * numRectangles );		recipient.texture_coords[ bottomIdx + offset ] = vec2( tileNumZ*topIdx / numRectangles, 0 );
...
function cube( tileNumX=1, tileNumZ=1 )		
	{
		shape.call(this);
		this.populate( this, mat4(), tileNumX, tileNumZ );
		this.init_buffers();
	}
inherit(cube, shape);

	cube.prototype.populate = function( recipient, points_transform, tileNumX, tileNumZ )
	{
			var m_strip = new rectangular_strip(); 
			for( var i = 0; i < 3; i++ )										// Build a cube by inserting six triangle strips into the lists.
				for( var j = 0; j < 2; j++ )
					m_strip.populate( recipient, 1, mult( points_transform, mult( rotate( 90, vec3( i==0, i==1, i==2 ) ), translate( j - .5, 0, 0 ) ) ), tileNumX, tileNumZ);
	}
